...
const AttendanceForm = ({ editingItem, onClose }) => {
  ...
  const handleSubmit = (values, { setSubmitting }) => {
    console.log('Attendance form submitted:', values);
    setTimeout(() => {
      setSubmitting(false);
      onClose();
    }, 1000);
  ...
  return (
    <Formik
      ...
      validationSchema={validationSchemas.attendance}
      onSubmit={handleSubmit}
    >
      {({ isSubmitting }) => (
        <Form className="space-y-4 md:space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {formTemplates.attendance.map(field => (
              <div 
                key={field.name} 
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4 md:pt-6">
            ...
            >
              {isSubmitting ? (
                ...
                <span>{editingItem ? 'Update Attendance' : 'Record Attendance'}</span>
              )}
            </button>
            <button
              type="button"
